...
export default function EditNews() {
  ...
  const fetchNews = async (newsId: string) => {
    try {
      const response = await axios.get(`https://newsweb-backend.vercel.app/news/${newsId}`);
  ...
  const handleUpdateNews = async (e: React.FormEvent) => {
    ...
    setSuccess('');
    try {
      await axios.put(`https://newsweb-backend.vercel.app/news/update/${id}`, { title, content });
